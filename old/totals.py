...
data['sanitarians']['value'] = data['sanitarians']['value'].apply(
    lambda x: "{:,}".format(x).replace(',', '.'))

data['residences'] = cases[['Fecha', 'Residencias Activos']]
data['residences'] = data['residences'].melt(
    id_vars=['Fecha'], var_name='Variables')
data['residences']['value'] = data['residences']['value'].astype(int)
data['residences']['value'] = data['residences']['value'].apply(
    lambda x: "{:,}".format(x).replace(',', '.'))
