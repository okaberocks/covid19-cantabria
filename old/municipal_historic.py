...
for date in dates:
    ...
    for municipio in municipios:
        ...
        casos_municipio = data_municipio[[
            'Fecha', 'Municipio', 'NumeroCasos', 'NumeroFallecidos', 'poblacion']]
        poblacion_municipio = casos_municipio['poblacion'].tail(1).values[0]

        casos_municipio['Casos nuevos7'] = casos_municipio['NumeroCasos'].diff(
            periods=7)
        casos_municipio['Casos nuevos14'] = casos_municipio['NumeroCasos'].diff(
            periods=14)
        ...
        casos_municipio['Incidencia acumulada 7 días'] = (
            casos_municipio['Casos nuevos7'] / casos_municipio['poblacion']) * 100000
        casos_municipio['Incidencia acumulada 14 días'] = (
            casos_municipio['Casos nuevos14'] / casos_municipio['poblacion']) * 100000

        municipio_tocsv = casos_municipio[[
            'Fecha', 'Municipio', 'Incidencia acumulada 14 días']].iloc[13:]
        municipio_tocsv['Fecha'] = pd.to_datetime(
            municipio_tocsv['Fecha'], dayfirst=True).dt.strftime('%Y-%m-%d')
        municipio_tocsv = municipio_tocsv.pivot(
            index='Municipio', columns='Fecha', values='Incidencia acumulada 14 días')
        ...
        if (poblacion_municipio > 5000):
            ...
            municipio_tocsv.insert(
                1, 'Poblacion', 'Entre 1000 y 5000 habitantes')
        else:
            municipio_tocsv.insert(1, 'Poblacion', 'Menos de 1000 habitantes')

        incidence_all = incidence_all.append(
            municipio_tocsv, ignore_index=True, sort=False)

        incidencia_municipio14 = casos_municipio[[
            'Municipio', 'Incidencia acumulada 14 días']].loc[casos_municipio['Fecha'] == date]
        incidencia_municipios14 = incidencia_municipios14.append(
            incidencia_municipio14, ignore_index=True, sort=False)

        incidencia_municipio7 = casos_municipio[[
            'Municipio', 'Incidencia acumulada 7 días']].loc[casos_municipio['Fecha'] == date]
        incidencia_municipios7 = incidencia_municipios7.append(
            incidencia_municipio7, ignore_index=True, sort=False)

        incidencia_municipio14 = casos_municipio[[
            'Fecha', 'Municipio', 'Incidencia acumulada 14 días']]
        incidencia_municipio7 = casos_municipio[[
            'Fecha', 'Municipio', 'Incidencia acumulada 7 días']]

        casos_diarios_municipio = casos_municipio[[
            'Fecha', 'Municipio', 'Casos nuevos']]
        fallecidos_diarios_municipio = casos_municipio[[
            'Fecha', 'Municipio', 'Fallecidos diarios']]
    ...
    utils.publish_firebase('saludcantabria/incidencia/' + date,
                           'incidencia-acumulada', datasets['Incidencia acumulada'])
    ...
    utils.publish_firebase('saludcantabria/incidencia/' + date,
                           'incidencia-acumulada-7', datasets['Incidencia acumulada 7'])
    break
